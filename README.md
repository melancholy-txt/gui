# Build a React Native Cookie Clicker (Tutorial: Steps 1-4)

This is a **build-along tutorial** for the first 4 steps of your plan:
1. Project setup
2. Core click mechanic (`useState` + UI)
3. Tap animation (`Animated`)
4. Passive income (`useEffect` + `setInterval`)

Instead of pasting full files, you will build the app in small, clear increments.

---

## What you will build by the end of Step 4

- A tappable cookie that increases score
- Animated press feedback (cookie scales down/up)
- A basic upgrade ("Grandma") that adds cookies per second
- A stable game loop with interval cleanup
...
```powershell
New-Item -ItemType Directory -Force -Path src, src\components, src\screens, src\utils
```
